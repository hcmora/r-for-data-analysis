...
# Let's check how many variables would be the best to use in mtry
oob.err = double(13)
test.err = double(13)
for(mtry in 1:13){
...
# It would seem that mtry = 10 would give the lowest test error.
...
# We can create a matrix of predictions based on the number of trees
boost.boston = gbm(medv~.,data=Boston[train,],distribution="gaussian",n.trees=10000,interaction.depth=4)
n.trees = seq(from=100,to=10000,by=100)
predmat = predict(boost.boston,newdata=Boston[-train,],n.trees=n.trees)
dim(predmat)
# Compute columnwise mean squared error
berr = with(Boston[-train,],apply((predmat-medv)^2,2,mean))
par(mfrow=c(1,1))
plot(n.trees,berr,pch=19,ylab="Mean Squared Error",xlab="# Trees",main = "Boosting Test Error")
# Comparison with the lowest test error from random forest
